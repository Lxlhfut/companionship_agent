...
def get_reminders(user_id):
    ...
    return [(row['id'], row['medicine_name'], row['dosage'], row['time_of_day'], row['days_of_week'], row['active']) for
            row in rows] if rows else []
...
def get_reminder_by_id(reminder_id):
    ...
    return (row['id'], row['medicine_name'], row['dosage'], row['time_of_day'], row['days_of_week'],
            row['active']) if row else None
...
def get_alert_rules(user_id):
    ...
    rows = execute_sql(sql, (user_id,), fetch_all=True)
    return [(row['id'], row['user_id'], row['rule_type'], row['threshold'], row['notification_method'], row['enabled'])
